...
@cli.command()
@click.option("--env", type=click.Choice(["test", "dev", "prod"]), default="dev", help="Environment to run")
...
@db.command()
@click.option("--env", type=click.Choice(["test", "dev", "prod"]), default="dev", help="Environment")
def migrate(env: str):
    ...
    try:
        ...
        if applied_dict:
            ...
            for filename, migration in sorted(applied_dict.items()):
                applied_time = migration.applied_at.strftime("%Y-%m-%d %H:%M:%S") if migration.applied_at else "unknown"
...
async def _create_dev_db():
    ...
    try:
        ...
        exists = await conn.fetchval(
            "SELECT 1 FROM pg_database WHERE datname = 'llmring_dev'"
        )
